...
export function serializeDate(value) {
    ...
    // if (org_value.locale() == 'en'){
    //     result= moment(org_value).format(SERVER_DATETIME_FORMAT_JALALI)
    // }

    // if (!dateCache.has(org_value)) {
    //     dateCache.set(org_value, org_value);
    // }
...
export function serializeDateTime(value) {
    ...
    const utc_value = org_value.utc();
    console.log(org_value + " is converted to " + utc_value);
        
    if (!dateTimeCache.has(org_value)) {
        dateTimeCache.set(
            org_value,
            utc_value
        );
...
export function deserializeDate(value) {
    ...
    const org_value = moment(value, 'jYYYY-jMM-jDD HH:mm:ss');
    return org_value;
...
export function deserializeDateTime(value) {
    ...
    if (moment(value).locale() == 'en'){
        const org_en = moment.utc(value);
        org_en = org_en.format('jYYYY-jMM-jDD HH:mm:ss');
        return org_en;
    }
    else{
        const org_en = moment.utc(value);
        org_fa = moment(org_en).format('jYYYY-jMM-jDD HH:mm:ss');
        return org_fa;
    }
    // const org_value = moment(value, SERVER_DATETIME_FORMAT_JALALI);
    // const org_value2 = moment(value, SERVER_DATETIME_FORMAT);
    // // if (org_value.locale() == '')
    // const local_value = org_value.toLocaleString();
    // const local_value2 = org_value2.toLocaleString();
    // return local_value;
