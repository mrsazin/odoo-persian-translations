...
class ResCompany(models.Model):
    ...
    def _compute_max_tax_lock_date(self):
        for company in self:
            company.max_tax_lock_date = max(company.tax_lock_date or JalaliDate.min, company.parent_id.sudo().max_tax_lock_date or JalaliDate.min)
    ...
    def _get_user_fiscal_lock_date(self):
        ...
        lock_date = max(self.period_lock_date or JalaliDate.min, self.fiscalyear_lock_date or JalaliDate.min)
        if self.user_has_groups('account.group_account_manager'):
            lock_date = self.fiscalyear_lock_date or JalaliDate.min
        if self.parent_id:
    ...
    def compute_fiscalyear_dates(self, current_date):
        ...
        return {'date_from': JalaliDatetime(year=current_date.year, month=1, day=1).date(),
                'date_to': JalaliDatetime(year=current_date.year, month=12, day=31).date()}
