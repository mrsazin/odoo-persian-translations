...
class MailActivity(models.Model):
    ...
    def create(self, vals_list):
        ...
        todo_activities = activities.filtered(lambda act: fields.Date.to_date(act.date_deadline) <= fields.Date.today())
        if todo_activities:
    ...
    def write(self, values):
        ...
        if values.get('user_id'):
            ...
            todo_activities = user_changes.filtered(lambda act: fields.Date.to_date(act.date_deadline) <= fields.Date.today())
            if todo_activities:
                self.env['bus.bus']._sendmany([
    ...
    def unlink(self):
        todo_activities = self.filtered(lambda act: fields.Date.to_date(act.date_deadline) <= fields.Date.today())
        if todo_activities:
            self.env['bus.bus']._sendmany([
                [partner, 'mail.activity/updated', {'activity_deleted': True}]
                for partner in todo_activities.user_id.partner_id
